...
void BRD_ADC0_Init(ADCx_InitTypeDef *ADCxInitStruct)
{
  CLKCTRL_PER1_CLKcmd(CLKCTRL_PER1_CLK_MDR_ADC0_EN, ENABLE);    
	ADCx_CLK_en(ADC0, ADCx_CLKSRC_MAX_CLK, ADCx_CLKdiv1);			
	ADC0->KEY = 0x8555AAA1;
  
  
	ADC_Init(ADC0, ADCxInitStruct);
	ADCx_Cmd(ADC0, ENABLE);
}

uint16_t BRD_ADCx_GetValue(ADCxControl* ADCx)
{	 
	ADCx_Start(ADCx);
	while(ADC_GetFlagStatus(ADCx, ADC_FLAG_NE)==RESET){}
  
  return ADCx_GetResult(ADCx);    	
}

void BRD_ADCx_StartAndWaitCompleted(ADCxControl* ADCx)
{	 
	ADCx_Start(ADCx);
	while(ADC_GetFlagStatus(ADCx, ADC_FLAG_NE)==RESET){}
}
